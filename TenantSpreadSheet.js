...
import IntelliSearch from './PairSearch';
...
export default class TenantSpreadSheet extends React.Component {
  ...
  saveEdit = (key) => {
    const newData = [...this.state.dataSource]; // get the current real-time data;
    const target = newData.filter(item => key === item.key)[0];
    const isRecordValid = this.isRecordValid(target);
    this.setState({
      isRecordValid,
    });
    if (!isRecordValid) {
  ...
  render() {
    let names = [], tenantAccounts = [], info = [];
      this.state.dataSource.forEach((val) => {
          names.push(val["Environment Name"]);
          tenantAccounts.push(val["Landscape"]);
          info.push(val["AWS Info"]);
      })
      console.log(names);
    ...
    return (
      <div style={{ margin: '32px 16px' }}>
        <div style={{ textAlign: 'left' }}>
          ...
          />
          <a size="large" onClick={() => { this.toggleAdvancedPanel(); }}>
            {this.state.advancedShown ? 'Hide' : 'Advanced'}</a>
